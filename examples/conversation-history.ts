...
async function main() {
  ...
  try {
    ...
    console.log(`  Tokens: in=${turn1Result.usage.inputTokens ?? 'N/A'}, out=${turn1Result.usage.outputTokens ?? 'N/A'}`);
    console.log();
    ...
    console.log(`  Tokens: in=${turn2Result.usage.inputTokens ?? 'N/A'}, out=${turn2Result.usage.outputTokens ?? 'N/A'}`);
  } catch (error) {
