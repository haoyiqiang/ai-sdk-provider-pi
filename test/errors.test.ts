...
describe('handlePiError', () => {
  ...
  it('converts context overflow errors', () => {
    try {
      handlePiError(new Error('prompt is too long'), { provider: 'anthropic', modelId: 'claude-sonnet-4' });
    } catch (e) {
  ...
  it('converts timeout errors', () => {
    try {
      handlePiError(new Error('Request timed out after 30s'), { provider: 'openai' });
    } catch (e) {
      expect(e).toBeInstanceOf(APICallError);
  ...
  it('converts generic errors to APICallError', () => {
    try {
      handlePiError(new Error('Something unexpected'), { provider: 'anthropic', modelId: 'claude-sonnet-4' });
    } catch (e) {
      expect(e).toBeInstanceOf(APICallError);
