...
describe('parseModelId', () => {
  ...
  it('resolves "haiku" alias', () => {
    ...
    expect(result).toEqual({ provider: 'anthropic', modelId: 'claude-haiku-4' });
  });
...
describe('validateModelSettings', () => {
  ...
  it('warns about both tools and excludeTools', () => {
    const result = validateModelSettings({ tools: ['bash'], excludeTools: ['edit'] });
    expect(result.warnings).toHaveLength(1);
...
describe('validateModelAvailability', () => {
  it('returns warning when model is not found in registry', () => {
    ...
    const result = validateModelAvailability('anthropic', 'unknown-model', registry);
    expect(result.warnings).toHaveLength(1);
  ...
  it('returns warning when API key is not configured', () => {
    ...
    const result = validateModelAvailability('anthropic', 'claude-sonnet-4', registry);
    expect(result.warnings).toHaveLength(1);
